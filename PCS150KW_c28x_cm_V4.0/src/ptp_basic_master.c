...
static void Ethernet_releaseTxPacketBufferCustom(
...
void ptp_master_init()
{
    ...
    initInterfaceConfig.phyMode = ETHERNET_SS_PHY_INTF_SEL_MII;  // 设置为MII模式
    initInterfaceConfig.clockSel = ETHERNET_SS_CLK_SRC_EXTERNAL; // 时钟源外部提供
    ...
    Ethernet_setMACConfiguration(EMAC_BASE, ((uint32_t)1 << 14));  // 设置 FES = 1 (100Mbps)
    Ethernet_setMACConfiguration(EMAC_BASE, ((uint32_t)1 << 13));  // 设置 DM = 1 (全双工模式)
