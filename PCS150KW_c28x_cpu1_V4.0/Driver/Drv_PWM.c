...
#define EPWM_DEAD_TIME              250 //死区时间2.4us 240*10
...
void Drv_PwmPin_Init(void)
{
    ...
    GPIO_setPinConfig(EPWM1_EPWMA_PIN_CONFIG);//配置PIN复用模式
...
//        //设置GPIO28为XBAR_OUTPUT5
//        GPIO_setPinConfig(GPIO_28_OUTPUTXBAR5);
//
//        //设置XBAR_OUTPUT5的信号源为MUX14的EXTSYNCOUT
...
//    else
//    {
//        // 设置EXTSYNCIN1信号作为EPWM1的同步信号；EXTSYNCIN1固定接到XBAR_INPUT5，EXTSYNCIN2固定接到XBAR_INPUT6
...
//    SysCtl_setSyncInputConfig(SYSCTL_SYNC_IN_ECAP6, SYSCTL_SYNC_IN_SRC_EPWM1SYNCOUT);
    ...
    SysCtl_setSyncOutputConfig(SYSCTL_SYNC_OUT_SRC_EPWM1SYNCOUT);
    // SOCA
    SysCtl_enableExtADCSOCSource(SYSCTL_ADCSOC_SRC_PWM1SOCA);//设置SOCA的源
    // SOCB
...
static void Drv_EPWMx_Init(uint32_t PwmXBase,uint32_t PwmPrd,uint32_t PhaseShift,uint32_t ComPareA,uint32_t ComPareB,int16_t DeadZEn,int16_t SyncEn)
{
    ...
    EPWM_setTimeBasePeriod(PwmXBase, PwmPrd);   //设置定时周期
...
//    EPWM_disablePhaseShiftLoad(PwmXBase); //关闭相位偏移加载
    //当同步信号到来，计数器Load相移值
    EPWM_setPhaseShift(PwmXBase, PhaseShift);//相位偏移
    EPWM_enablePhaseShiftLoad(PwmXBase);
    // 设置SyncOut为SyncIn，即PWM1产生的同步信号bypassP PWM2直接输出
...
//    EPWM_enableSyncOutPulseSource(PwmXBase, EPWM_SYNC_OUT_PULSE_ON_CNTR_ZERO);
    ...
    if(DeadZEn)
    {
        EPWM_setDeadBandCounterClock(PwmXBase,EPWM_DB_COUNTER_CLOCK_FULL_CYCLE);//时钟周期100M 10ns
        EPWM_setRisingEdgeDelayCount(PwmXBase,EPWM_DEAD_TIME);//设置死区时间
        EPWM_setFallingEdgeDelayCount(PwmXBase,EPWM_DEAD_TIME);
        EPWM_setDeadBandDelayMode(PwmXBase,EPWM_DB_RED,true);//死区使能
        EPWM_setDeadBandDelayMode(PwmXBase,EPWM_DB_FED,true);
        EPWM_setRisingEdgeDeadBandDelayInput(PwmXBase,EPWM_DB_INPUT_EPWMA);//上升沿死区计数器输入选为EPWMA
        EPWM_setFallingEdgeDeadBandDelayInput(PwmXBase,EPWM_DB_INPUT_EPWMA);//下降沿死区计数器输入选为EPWMA
        EPWM_setDeadBandDelayPolarity(PwmXBase,EPWM_DB_RED,EPWM_DB_POLARITY_ACTIVE_HIGH);//设置PWM上升沿死区计数后不反转极性
        EPWM_setDeadBandDelayPolarity(PwmXBase,EPWM_DB_FED,EPWM_DB_POLARITY_ACTIVE_LOW);//设置PWM下降沿死区计数后反转极性
    }
    //TZ封锁PWM
...
//    EPWM_setActionQualifierContSWForceShadowMode(PwmXBase,EPWM_AQ_SW_IMMEDIATE_LOAD);//先强制输出低，不输出
...
//    EPWM_setActionQualifierContSWForceAction(PwmXBase,EPWM_AQ_OUTPUT_B,EPWM_AQ_SW_OUTPUT_LOW);
    ...
    EPWM_disableChopper(PwmXBase);//关闭斩波功能
}

static void Drv_EPWM1_Init(void)
{
/*    //当PWM1产生的同步信号到来，计数器Load相移值
//    EPWM_setPhaseShift(EPWM1_BASE, 0);//0度
...
//    }*/
    ...
    EPWM_setADCTriggerSource(EPWM1_BASE, EPWM_SOC_A, EPWM_SOC_TBCTR_ZERO_OR_PERIOD);//ADC出发频率32KHZ
...
//    EPWM_setActionQualifierContSWForceAction(EPWM7_BASE,EPWM_AQ_OUTPUT_A,EPWM_AQ_SW_OUTPUT_LOW); //强制输出低
//    EPWM_setActionQualifierContSWForceAction(EPWM7_BASE,EPWM_AQ_OUTPUT_B,EPWM_AQ_SW_OUTPUT_LOW); //强制输出低
}
...
{
    EPWM_setActionQualifierContSWForceShadowMode(EPWM9_BASE,EPWM_AQ_SW_IMMEDIATE_LOAD);
//    EPWM_setActionQualifierContSWForceAction(EPWM16_BASE,EPWM_AQ_OUTPUT_A,EPWM_AQ_SW_OUTPUT_LOW); //强制输出低
    EPWM_setActionQualifierContSWForceAction(EPWM9_BASE,EPWM_AQ_OUTPUT_A,EPWM_AQ_SW_OUTPUT_LOW); //强制输出低
//    EPWM_setActionQualifierContSWForceShadowMode(EPWM16_BASE,EPWM_AQ_SW_SH_LOAD_ON_CNTR_ZERO_PERIOD);
}
